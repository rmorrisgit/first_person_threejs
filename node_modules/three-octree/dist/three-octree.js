 ...
 *
 */
 ( function ( THREE$$1 ) { function isNumber ( n ) {
	...
	THREE$$1.Octree = function ( parameters ) {
		...
		if ( this.scene ) {
			...
			this.visualGeometry = new THREE$$1.CubeGeometry( 1, 1, 1 );
			this.visualMaterial = new THREE$$1.MeshBasicMaterial( { color: 0xFF0066, wireframe: true, wireframeLinewidth: 1 } );
		...
		this.root = parameters.root instanceof THREE$$1.OctreeNode ? parameters.root : new THREE$$1.OctreeNode( parameters );

	};

	THREE$$1.Octree.prototype = {
		...
		addDeferred: function ( object, options ) {
			...
			if ( object instanceof THREE$$1.OctreeObjectData ) {
		...
		addObjectData: function ( object, part ) {
			...
			var objectData = new THREE$$1.OctreeObjectData( object, part );
		...
		extend: function ( octree ) {
			...
			if ( octree instanceof THREE$$1.Octree ) {
		...
		rebuild: function () {
			...
			for ( i = 0, l = this.objectsData.length; i < l; i++ ) {
				...
				if ( node instanceof THREE$$1.OctreeNode && !objectData.positionLast.equals( objectData.position ) ) {
		...
		setRoot: function ( root ) {
			...
			if ( root instanceof THREE$$1.OctreeNode ) {
	...
	THREE$$1.OctreeObjectData = function ( object, part ) {
		...
		} else if ( part instanceof THREE$$1.Face4 ) {
	...
	THREE$$1.OctreeObjectData.prototype = {
	...
	THREE$$1.OctreeNode = function ( parameters ) {
		...
		if ( parameters.tree instanceof THREE$$1.Octree ) {

			this.tree = parameters.tree;

		} else if ( parameters.parent instanceof THREE$$1.OctreeNode !== true ) {

			parameters.root = this;

			this.tree = new THREE$$1.Octree( parameters );
		...
		if ( this.tree.scene ) {
			...
			this.visual = new THREE$$1.Mesh( this.tree.visualGeometry, this.tree.visualMaterial );
	...
	THREE$$1.OctreeNode.prototype = {
		...
		updateProperties: function () {
			...
			if ( this.parent instanceof THREE$$1.OctreeNode ) {
		...
		addObject: function ( object ) {
			...
			} else if ( indexOctant < -1 && this.parent instanceof THREE$$1.OctreeNode ) {
		...
		branch: function ( indexOctant ) {
			...
			if ( this.nodesByIndex[ indexOctant ] instanceof THREE$$1.OctreeNode ) {
		...
		checkMerge: function () {
			...
			while ( nodeParent.parent instanceof THREE$$1.OctreeNode && nodeParent.getObjectCountEnd() < this.tree.objectsThreshold ) {
		...
		checkContract: function () {
			...
			if ( this.nodesIndices.length > 0 ) {
				...
				for ( i = 0, l = this.nodesIndices.length; i < l; i++ ) {
					...
					if ( nodeHeaviest instanceof THREE$$1.OctreeNode === false || nodeObjectsCount > nodeHeaviestObjectsCount ) {
				...
				if ( outsideHeaviestObjectsCount < this.tree.objectsThreshold && nodeHeaviest instanceof THREE$$1.OctreeNode ) {
		...
		getOctantIndex: function ( objectData ) {
			...
			if ( objectData instanceof THREE$$1.OctreeObjectData ) {
			...
			} else if ( objectData instanceof THREE$$1.OctreeNode ) {
		...
		getObjectCountStart: function () {
			...
			while( parent instanceof THREE$$1.OctreeNode ) {
